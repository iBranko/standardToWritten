...
let separateByPlaceValue = function (number) {
...
let standardToWritten = function (n) {
    let a = separateByPlaceValue(n).split(",");
    let sizea = a.length;
    let writtenNumber = "";
    
    for(let i = 0; i < sizea; i++) {
        if(a[i] != "000") {
            let b = a[i].toString().split("");
            b.reverse();
            let sizeb = b.length;
            
            for(let j = sizeb-1; j >= 0; j--){
                if(i > 0 && j === sizeb-1){
                    writtenNumber += ", ";
                }
